...
async function showGitHubIssueForm() {
  try {
    ...
    // Set a smart title from captured context
...
function handleRepoSelected(e) {
  ...
  if (selectedRepo) {
    ...
    // Fetch labels and next issue number for the selected repo
    const [owner, repo] = selectedRepo.full_name.split('/');
    fetchRepoLabels(owner, repo);

    // Set default title to "Issue #N" with optional context suffix
    GitHubService.getLatestIssueNumber(owner, repo).then(latestNum => {
      const nextNum = latestNum + 1;
      const currentTitle = issueTitle.value.trim();
      const context = generateSmartTitle();
      // Only set if user hasn't typed a custom title
      // Overwrite: empty, 'Voice Note', existing 'Issue #N...' patterns, or raw context strings
      const isAutoGenerated = !currentTitle || currentTitle === 'Voice Note'
        || currentTitle.match(/^Issue #\d+/)
        || currentTitle === context;
      if (isAutoGenerated) {
        const suffix = (context && context !== 'Voice Note') ? `- ${context}` : '';
        issueTitle.value = `Issue #${nextNum}${suffix ? ' ' + suffix : ''}`;
      }
    }).catch(() => { /* ignore - title stays as-is */ });
  }
}
...
async function fetchRepoLabels(owner, repo) {
  ...
  try {
    ...
    // Re-add existing selected labels with real color info from repo
    const previousLabels = [...selectedLabels];
    selectedLabels = [];
    selectedLabelsContainer.innerHTML = '';
    for (const tag of previousLabels) {
...
async function handleCreateIssue() {
  try {
    ...
    body += `\n\n---\n*\u2014noted by [Duly Noted](https://dulynoted.xyz)*`;
 ...
 * Returns a plain descriptive string (no [Bug] prefix, no issue number).
 * The issue number is added by handleRepoSelected when a repo is chosen.
 */
  ...
  if (capturedElement) {
    ...
    try {
      const hostname = capturedScreenshots[0]?.tabUrl
        ...
        : '';
      return hostname ? `${tag}${id} on ${hostname}` : `${tag}${id}`;
    } catch {
      return `${tag}${id}`;
    }
  }
